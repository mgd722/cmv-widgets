define([
  ...
  return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    polygon: {
        outlineColor: [255, 0, 0, 1.0],
        outlineThickness: 2,
        shadingColor: [255,0,0,0.2]
    },
    ...
    handleDrop: function(event) {
      ...
      if (files && files.length === 1) {
        ...
        if (file.name.toLowerCase().indexOf('.kml') !== -1 || file.name.toLowerCase().indexOf('.kmz') !== -1) {
          // convert kml to zipped shape
          var redirect = window.confirm('KMLs and KMZs currently not supported.  Open web converter?');
          if (redirect === true){
            window.open('http://www.zonums.com/online/kml2shp.php');
          }
        }
    ...
    changeShapefileRenderer: function(layer) {
      ...
      'longdash': SimpleLineSymbol.STYLE_LONGDASH,
      'longdashdot': SimpleLineSymbol.STYLE_LONGDASHDOT,
      'null': SimpleLineSymbol.STYLE_NULL,
      'shortdash': SimpleLineSymbol.STYLE_SHORTDASH,
      'shortdashdot': SimpleLineSymbol.STYLE_SHORTDASHDOT,
      'shortdashdotdot': SimpleLineSymbol.STYLE_SHORTDASHDOTDOT,
      'shortdot': SimpleLineSymbol.STYLE_SHORTDOT,
        ...
        case 'esriGeometryPolygon':
          symbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,
            new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,
              new Color(this.polygon.outlineColor), this.polygon.outlineThickness), new Color(this.polygon.shadingColor));
          break;
        case 'esriGeometryPolyline':
          symbol = new SimpleLineSymbol(styles[this.line.style.toLowerCase()], new Color(this.line.color), this.line.thickness);
          break;
      }
      if (symbol) {
